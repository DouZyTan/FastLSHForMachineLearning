...
void sortQueryPointsByRadii(IntT dimension,
  ...
  //for each query to obtain the distance with its NN
...
//     }
    ...
    for(IntT rep = 0; rep < nReps; rep++){
      ...
      timeCycleBucket = 0;
      nOfDistComps = 0;
      nNNs = getNearNeighborsFromPRNearNeighborStruct(nnStruct, queryPoint, result, resultSize, num);
...
RNNParametersT computeOptimalParameters(RealT R, 
  ...
  for(k = 2; ; k += 2){
    ...
    // std::cout<<"current k value: "<<k<<std::endl;
